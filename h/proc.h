...
struct	pentry	{
	...
	int pinh;				/* Inherited priority for a process */
	int plockid;			/* lock descriptor in whose wait queue the process is blocked */
	int plockheld[NLOCKS];	/* Locks held and their status */
	int plstartwait;		/* wait start time entering the lock queue */

	int plwaitprio;			/* waiting priority of process for the lock */	
	int ltype;				/* type of lock for the process */
};
